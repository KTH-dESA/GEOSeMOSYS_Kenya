...
def peakdemand(outPutFile,input_data, peakdemand):
    ...
    for j, row in demand.iterrows():
        ...
        for k in year: #year is an object so I cannot match it with a number (e.g. startyear)
            ...
            dataToInsert += "%s\t%s\t%s\t%s\n" % (input_data['region'][0], j, k, round(demandForThisYearAndlocation,2))
    outPutFile = outPutFile[:startIndex] + dataToInsert + outPutFile[startIndex:]
    return(outPutFile)
                ...
                for t in solar_tech:
                    if t == 'SOPV':
                        if elec['pointid'].eq(row['Location']).any():
                            dataToInsert += "%s\t%s_%s_1\t%s\t%i\t%f\n" % (region, t, location, tsday, year, round(average_solar_day,2))
                            dataToInsert += "%s\t%s_%s_0\t%s\t%i\t%f\n" % (region, t, location, tsday, year, round(average_solar_day,2))
                            dataToInsert += "%s\t%s_%s_1\t%s\t%i\t%f\n" % (region, t, location, tsevening, year, round(average_solar_evening,2))
                            dataToInsert += "%s\t%s_%s_0\t%s\t%i\t%f\n" % (region, t, location, tsevening, year, round(average_solar_evening,2))
                            dataToInsert += "%s\t%s_%s_1\t%s\t%i\t%f\n" % (region, t, location, tsnight, year, round(average_solar_night,2))
                            dataToInsert += "%s\t%s_%s_0\t%s\t%i\t%f\n" % (region, t, location, tsnight, year, round(average_solar_night,2))
                ...
                while year <= endyear:
                    ...
                    while m < len(timeslice):
                        ...
                        if line['Technology'] == 'SOPV':
                            if elec['pointid'].eq(row['Location']).any():
                                dataToInsert += "%s\t%s%ir_%s_1\t%s\t%i\t%f\n" % (region, line['Technology'], line['BatteryTime'], location, tsday, year, round(average_solar_day,2))
                                dataToInsert += "%s\t%s%ir_%s_0\t%s\t%i\t%f\n" % (
                                region, line['Technology'], line['BatteryTime'], location, tsday, year, average_solar_day)
                                dataToInsert += "%s\t%s%ir_%s_1\t%s\t%i\t%f\n" % (region, line['Technology'], line['BatteryTime'], location, tsevening, year, round(average_solar_evening,2))
                                dataToInsert += "%s\t%s%ir_%s_0\t%s\t%i\t%f\n" % (
                                region, line['Technology'], line['BatteryTime'], location, tsevening, year, average_solar_evening)
                                dataToInsert += "%s\t%s%ir_%s_1\t%s\t%i\t%f\n" % (region, line['Technology'], line['BatteryTime'], location, tsnight, year, round(average_solar_night,2))
                                dataToInsert += "%s\t%s%ir_%s_0\t%s\t%i\t%f\n" % (
                                region, line['Technology'], line['BatteryTime'], location, tsnight, year, round(average_solar_night,2))
                            if un_elec['pointid'].eq(row['Location']).any():
                                dataToInsert += "%s\t%s%ir_%s_0\t%s\t%i\t%f\n" % (region, line['Technology'], line['BatteryTime'], location, tsday, year, round(average_solar_day,2))
    ...
    for k, row in df.iterrows():
        ...
        while year <= endyear:
            ...
            while m < len(timeslice):
                ...
                for t in wind_tech:
                    dataToInsert += "%s\t%s_%s\t%s\t%i\t%f\n" % (region, t, location , tsday, year, round(average_wind_day,2))
                    dataToInsert += "%s\t%s_%s\t%s\t%i\t%f\n" % (region, t, location, tsevening, year, round(average_wind_evening,2))
                    dataToInsert += "%s\t%s_%s\t%s\t%i\t%f\n" % (region, t, location, tsnight, year, round(average_wind_night,2))
                m = m + 1
            year = year + 1
    # if battery is None:
    #     pass
...
def capitalcost(outPutFile, trade_cost, input_data):
    ...
    for m, row in trade_cost.iterrows():
        if row['Technology'] != "":
            ...
            year = int(input_data['startyear'][0])
            while year <= int(input_data['endyear'][0]):
                dataToInsert += "%s\t%s\t%i\t%f\n" % (input_data['region'][0], tech, year, round(cost,2))
                year += 1
