...
def capacityfactor(outPutFile, df, battery, input_data, capacityfactor_wind, capacityfactor_solar, elec):
    """
    ...
    capacityfactor_solar_p = pd.to_datetime(capacityfactor_solar_['time'], errors='coerce', format='%Y/%m/%d %H:%M')
    capacityfactor_solar_.index = capacityfactor_solar_p
    capacityfactor_solar_pv = capacityfactor_solar_.drop(columns=['time'])
    ...
    if battery is None:
        ...
        for j, line in solar_battery.iterrows():
            ...
            for k, row in df.iterrows():
                ...
                capacityfactor_solar_battery = capacityfactor_solar_b.drop(columns=['time'])
...
#WIND
    ...
    capacityfactor_windc = pd.to_datetime(capacityfactor_windcop['time'], errors='coerce', format='%Y/%m/%d %H:%M')
    capacityfactor_windcop.index = capacityfactor_windc
    capacityfactor_windcopy = capacityfactor_windcop.drop(columns=['time'])
    ...
    for k, row in df.iterrows():
        ...
        while year <= endyear:
            ...
            while m < 11:
                ...
                startDate = "2016-%s-01" % (currentMonth)
                endDate = "2016-%s-01" % (months[m + 1])
                thisMonthOnly = capacityfactor_windcopy.loc[startDate:endDate]
                sliceStart = timesliceDN
                sliceEnd = timesliceDE
                ts = "%iD" % (m + 1)
                slice = sum(thisMonthOnly[(location)].between_time(sliceStart, sliceEnd))
    ...
    if battery is None:
        ...
        for j, line in wind_battery.iterrows():
            ...
            for k, row in df.iterrows():
                ...
                capacityfactor_wind_battery = capacityfactor_wind_b.drop(columns=['time'])
                ...
                while year <= endyear:
                    ...
                    while m < 11:
                        ...
                        startDate = "2016-%s-01" % (currentMonth)
                        endDate = "2016-%s-01" % (months[m + 1])
                        thisMonthOnly = capacityfactor_wind_battery.loc[startDate:endDate]
                        sliceStart = timesliceDN
                        sliceEnd = timesliceDE
                        ts = "%iD" % (m + 1)
                        slice = sum(thisMonthOnly[(location)].between_time(sliceStart, sliceEnd))
...
def capitalcost_dynamic(df, outPutFile, capitalcost_RET, capacityfactor_wind, capacityfactor_solar, input_data, elec, battery):
    ...
    for m, row in df.iterrows():
       ...
       if battery_tech is None:
           ...
           for k in wind_tech.columns[3:]:  # year is an object so I cannot match it with a number (e.g. startyear)
               windcapitalcostbatt = wind_tech.loc[cf][k] + battery_tech.loc[13][k]
               techname = wind_tech_name + "13h"
