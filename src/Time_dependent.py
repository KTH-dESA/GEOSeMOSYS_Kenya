...
def capacityfactor(outPutFile, df, battery, input_data, capacityfactor_wind, capacityfactor_other, elec, un_elec):
    """
    ...
    df = capacityfactor_other.columns.drop('adjtime')
...
dict_df =load_csvs("run -reduced TS/ref")
outPutFile = make_outputfile("run -reduced TS/Kenya_basefile_reference.txt")
outPutFile = capacityfactor(outPutFile, dict_df['GIS_data'], dict_df['battery'], dict_df['input_data'],
                            dict_df['capacityfactor_other'], dict_df['capacityfactor_other'], dict_df['elec'],
                            dict_df['un_elec'])
write_to_file('Capacityfactors_other.txt', outPutFile)